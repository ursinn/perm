...
public class PermUtils {
	...
	private void checkUser_MySQL() {
		...
		ResultSet rs = Main.getInstance().getMySQL()
				.getResult("SELECT * FROM information_schema.tables WHERE table_schema = '"
						+ Main.getInstance().MySQL_Database + "' AND table_name = '" + Tabel + "' LIMIT 1;");
		try {
			while (rs.next()) {
				return;
			}
		} catch (SQLException e) {
			Main.getInstance().getMySQL().update("CREATE TABLE IF NOT EXISTS `" + Tabel
					+ "` (`id` int(11) NOT NULL, `UUID` varchar(64) NOT NULL, `Group` int(11) NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=latin1;");
			Main.getInstance().getMySQL().update("ALTER TABLE `" + Tabel + "` ADD PRIMARY KEY (`id`);");
			Main.getInstance().getMySQL()
					.update("ALTER TABLE `" + Tabel + "` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;");
		}
	}
	...
	private void checkPerm_MySQL() {
		...
		try {
			...
			Main.getInstance().getMySQL().update("CREATE TABLE IF NOT EXISTS `" + Tabel
					+ "` (`id` int(11) NOT NULL, `Permission` varchar(250) NOT NULL, `Group` int(11) NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=latin1;");
			Main.getInstance().getMySQL().update("ALTER TABLE `" + Tabel + "` ADD PRIMARY KEY (`id`);");
			Main.getInstance().getMySQL()
					.update("ALTER TABLE `" + Tabel + "` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;");
		}
	...
	private void checkUserPerm_MySQL() {
		...
		try {
			...
			Main.getInstance().getMySQL().update("CREATE TABLE IF NOT EXISTS `" + Tabel
					+ "` (`id` int(11) NOT NULL, `UUID` varchar(64) NOT NULL, `Permission` varchar(250) NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=latin1;");
			Main.getInstance().getMySQL().update("ALTER TABLE `" + Tabel + "` ADD PRIMARY KEY (`id`);");
			Main.getInstance().getMySQL()
					.update("ALTER TABLE `" + Tabel + "` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;");
		}
	...
	private void checkGroup_MySQL() {
		...
		try {
			...
			Main.getInstance().getMySQL().update("CREATE TABLE IF NOT EXISTS `" + Tabel
					+ "` (`id` int(11) NOT NULL, `Name` varchar(250) NOT NULL, `Prefix` varchar(250) NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=latin1;");
			Main.getInstance().getMySQL().update("ALTER TABLE `" + Tabel + "` ADD PRIMARY KEY (`id`);");
			Main.getInstance().getMySQL()
					.update("ALTER TABLE `" + Tabel + "` MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;");
		}
	...
	public class PlayerAPI {
		...
		private void InsertUserInDB() {
			String Tabel = Main.getInstance().TabelPrefix + "User";
			Main.getInstance().getMySQL()
					.update("INSERT INTO " + Tabel + " (UUID, Group) VALUES ('" + uuid + "', '1')");
		}
